...
+ (void)xlsn0w_updateVersionWithAppStoreID:(NSString *)appStoreID
    ...
    //确定请求路径
    NSURL *xlsn0wURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://itunes.apple.com/cn/lookup?id=%@", appStoreID]];
    ...
    */
    ...
    NSURLSessionDataTask *xlsn0wURLSessionDataTask = [xlsn0wURLSession dataTaskWithRequest:xlsn0wURLRequest completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        if (data == nil) {
            NSLog(@"您没有连接网络");
            [XLsn0wVersionManager xlsn0w_showVersionMessage:@"您没有连接网络 !"];
            return;
        }
        ...
        if (resultsArray.count < 1) {
            ...
            [XLsn0wVersionManager xlsn0w_showVersionMessage:@"此APPID为未上架的APP或者查询不到 !"];
            return;
        }
        ...
        BOOL isNewVersion = [XLsn0wVersionManager xlsn0w_compareVersionsFormAppStoreVersion:appStoreVersion currentAppVersion:currentVersion];
        ...
        if(isNewVersion == YES) {
            ...
            NSLog(@"您当前版本已经最新");
            [XLsn0wVersionManager xlsn0w_showVersionMessage:@"您当前版本已经最新 !"];
        }
    }];
...
+ (void)xlsn0w_showVersionMessage:(NSString *)versionMessage {
    ...
    messageHud.label.text = [versionMessage isKindOfClass:[NSString class]] ? versionMessage : @"";
...
+ (BOOL)xlsn0w_compareVersionsFormAppStoreVersion:(NSString*)AppStoreVersion currentAppVersion:(NSString*)currentAppVersion {
