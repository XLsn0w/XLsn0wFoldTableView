...
@interface XLsn0wCollectionViewFlowLayout () {
...
- (void)prepareSectionLayout:(NSUInteger)section withNumberOfItems:(NSUInteger)numberOfItems{
    ...
    if([self.xlDelegate respondsToSelector:@selector(collectionView:layout:referenceSizeForHeaderInSection:)]){
        ...
        CGSize headerSize = [self.xlDelegate collectionView:self.collectionView layout:self referenceSizeForHeaderInSection:indexPath.section];
    ...
    if([self.xlDelegate respondsToSelector:@selector(collectionView:layout:insetForSectionAtIndex:)]){
        sectionInset = [self.xlDelegate collectionView:cView layout:self insetForSectionAtIndex:section];
    }
    ...
    if([self.xlDelegate respondsToSelector:@selector(collectionView:layout:minimumInteritemSpacingForSectionAtIndex:)]){
        interitemSpacing = [self.xlDelegate collectionView:cView layout:self minimumInteritemSpacingForSectionAtIndex:section];
    }
    
    if([self.xlDelegate respondsToSelector:@selector(collectionView:layout:minimumLineSpacingForSectionAtIndex:)]){
        lineSpacing = [self.xlDelegate collectionView:cView layout:self minimumLineSpacingForSectionAtIndex:section];
    }
    
    itemsContentRect.origin.x = sectionInset.left;
    itemsContentRect.origin.y = headerHeight + sectionInset.top;
    
    NSUInteger numberOfColumns = [self.xlDelegate collectionView:cView layout:self numberOfColumnsInSection:section];
    ...
    for (NSInteger itemIdx = 0; itemIdx < numberOfItems; ++itemIdx) {
        ...
        CGSize itemSize = [self.xlDelegate collectionView:cView layout:self sizeForItemAtIndexPath:itemPath];
    ...
    if([self.xlDelegate respondsToSelector:@selector(collectionView:layout:referenceSizeForFooterInSection:)]){
        CGRect footerFrame;
        footerFrame.origin.x = 0;
        footerFrame.origin.y = itemsContentRect.size.height;
        
        CGSize footerSize = [self.xlDelegate collectionView:self.collectionView layout:self referenceSizeForFooterInSection:indexPath.section];
