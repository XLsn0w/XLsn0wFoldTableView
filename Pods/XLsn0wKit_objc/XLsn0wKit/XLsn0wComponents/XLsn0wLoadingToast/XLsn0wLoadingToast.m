...
@implementation XLsn0wLoadingToast {
    BOOL useAnimation;
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    XLsn0wLoadingToast *hud = [[self alloc] initWithView:view];
    [view addSubview:hud];
    [hud show:animated];
    return hud;
}
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    ...
    XLsn0wLoadingToast *hud = [self HUDForView:view];
    if (hud != nil) {
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    if (view == nil) view = [[UIApplication sharedApplication].windows lastObject];
    NSArray *huds = [XLsn0wLoadingToast allHUDsForView:view];
    for (XLsn0wLoadingToast *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hide:animated];
    }
    return [huds count];
...
+ (instancetype)HUDForView:(UIView *)view {
    ...
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (XLsn0wLoadingToast *)subview;
        }
    }
    return nil;
}
...
- (void)updateIndicators {
    ...
    if (mode == XLsn0wLoadingToastModeIndeterminate &&  !isActivityIndicator) {
        ...
        [indicator removeFromSuperview];
        self.indicator = [[XYQBarProgressView alloc] init];
        [self addSubview:indicator];
    }
        ...
        if (!isRoundIndicator) {
            ...
            [indicator removeFromSuperview];
            self.indicator = [[XYQRoundProgressView alloc] init];
            [self addSubview:indicator];
        }
...
- (void)layoutSubviews {
    ...
    //								constrainedToSize:maxSize lineBreakMode:detailsLabel.lineBreakMode]; //deprecated
...
@implementation XYQRoundProgressView
...
@implementation XYQBarProgressView
